...
local CART_COLOR = {
  ...
  crystal = PAL.railCrystal, ruby = PAL.railRuby,
}
...
-- The cartridges used to be tabs of their own.  They are one dropdown now:
-- the tab row wrapped to two rows on anything narrow, and only ever one game
-- is being looked at.  Built from GameVersion.ORDER so a new version cannot
-- ship in VERSIONS and stay missing from Choose game (Ruby Phase 1).
local TAB_LETTER = {
  red = "R", blue = "B", yellow = "Y", gold = "G",
  silver = "S", crystal = "C", ruby = "3",
}
local GAME_TABS = {}
for _, id in ipairs(GameVersion.ORDER) do
  local info = GameVersion.info(id)
  GAME_TABS[#GAME_TABS + 1] = {
    id = id,
    key = "tab-" .. id,
    letter = TAB_LETTER[id] or (info.label:sub(1, 1)),
    color = CART_COLOR[id] or PAL.green,
    label = info.launcherName or info.label,
  }
end
...
local function romModel(imp, version, info, ready, locked)
  ...
  local dropHint = imp.isNX and Strings("Copy the .gb/.gbc/.gba via MTP into imports/.")
    or (imp.baseRomDiscovery and Strings("Or copy the .gb/.gbc/.gba into baseroms/.")
      or (imp.android and Strings("Copy the .gb/.gbc/.gba via USB.")
  ...
  elseif scanning then
    return { state = Strings("Checking baseroms..."),
      detail = Strings("Looking for compatible Pokemon ROMs."),
