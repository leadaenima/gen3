...
-- .gb, .gbc, or .gba.  `%.gbc?$` is the existing Gen 1/2 match (.gb / .gbc).
local function isRomFilename(name)
  local lower = (name or ""):lower()
  return lower:match("%.gbc?$") ~= nil or lower:match("%.gba$") ~= nil
end

local function extractorFor(version)
  local gen = GameVersion.generation(version)
  if gen == 3 then return require("src.import.RomExtractorGen3") end
  if gen == 2 then return require("src.import.RomExtractorGen2") end
  return require("src.import.RomExtractor")
end
...
-- typically .gbc (Gold is 2 MiB); Ruby is a 16 MiB .gba.
...
function RomImporter.new(onComplete, opts)
  ...
  for _, version in ipairs(GameVersion.ORDER) do
    ...
    local ext = ".gb"
    local gen = GameVersion.generation(version)
    if gen == 3 then ext = ".gba"
    elseif info.id == "yellow" or gen == 2 then ext = ".gbc" end
...
function RomImporter:_startExtractCoroutine(version, info, prefix, displayName)
  self.worker = coroutine.create(function()
    ...
    local RomExtractor = extractorFor(version)
...
local CART_RAIL = { red = "railRed", blue = "railBlue", yellow = "railGold",
                    ...
                    crystal = "railCrystal", ruby = "railRuby" }
