...
-- pokeruby: 0 deg = 12 o'clock. LÖVE 0 rad = +X (3 o'clock), CW positive.
local function handRad(deg)
  return math.rad((tonumber(deg) or 0) - 90)
end
...
local function ampmOffset(deg)
  -- Cos2/Sin2 * 30 / 4096 with y+ down (screen space).
...
function WallClock.draw(game, f)
  ...
  if gfx.hands and gfx.qMinute and gfx.qHour then
    -- Hands point up in the sheet; rotate about sprite center.
    G.setColor(1, 1, 1, 1)
    G.draw(gfx.hands, gfx.qMinute, CENTER_X, CENTER_Y, handRad(minAng),
      1, 1, HAND_W / 2, HAND_H / 2)
    G.draw(gfx.hands, gfx.qHour, CENTER_X, CENTER_Y, handRad(hourAng),
      1, 1, HAND_W / 2, HAND_H / 2)
  end
