...
--
-- Android PhysFS is picky here: a zip folder may show up in
-- getDirectoryItems without getInfo reporting type "directory", and
-- getInfo(path, "file") can reject a zip entry that getInfo(path) accepts.
-- Probe manifest.json either way so a phone install does not report
-- "no manifest.json found in the zip" for an archive desktop mounted fine.
local function hasManifestFile(path)
...
local function topLevelPaths(mount)
  local fs = love.filesystem
  local paths = {}
  for _, name in ipairs(fs.getDirectoryItems(mount) or {}) do
    local info = fs.getInfo(mount .. "/" .. name)
    local looksLikeDir = info and info.type == "directory"
    if not looksLikeDir and hasManifestFile(mount .. "/" .. name .. "/manifest.json") then
      looksLikeDir = true
    end
    if looksLikeDir then
      if hasManifestFile(mount .. "/" .. name .. "/manifest.json") then
...
--
-- Android zip mounts often list a folder without getInfo reporting type
-- "directory". Treat a path as a directory when it has children so we recurse
-- instead of trying to read the folder name as a file.
local function isArchiveDir(path)
  local fs = love.filesystem
  local info = fs.getInfo(path)
...
function LauncherMods._installZipInner(source, opts)
  ...
  -- Android can return true from FileData mount while publishing no zip
  -- entries; treat an unlistable mount as a miss and fall back to a path
  -- mount so locateRoot does not report a missing manifest.json that is
  -- sitting in the archive.
  local mount = "mod_import_mount"
  local tmp = nil
  local mountKey = nil
  local mounted = false
  local function mountHasRoot()
    return LauncherMods.locateRoot(topLevelPaths(mount)) ~= nil
  end
