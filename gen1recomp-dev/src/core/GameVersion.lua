...
-- Yellow, Gold, Silver, Crystal, or Ruby.  One source of truth for everything
-- that differs by version -- the accepted ROM hash, the import manifest, where
-- the extracted cache lives, and the save-file suffix -- so the importer,
...
-- docs/gold-phase1.md); `generation` splits Gen 1 / Gen 2 / Gen 3 and `engine`
-- splits Gold/Silver from Crystal within Gen 2 (Ruby is `gen3`).
--
...
GameVersion.VERSIONS = {
  ...
  -- Gen 3, Phase 3 (docs/gen3-phase1.md): a 16 MiB GBA cart.  Import writes
  -- every Hoenn map plus shared tileset atlases.  Battle and the script
  -- VM are later engines, same split Gold used (Game2.lua beside Game.lua).
  ruby = {
    id = "ruby",
    label = "Ruby",
    displayName = "Pokemon Ruby",
    launcherName = "Ruby (Alpha)",
...
GameVersion.ORDER = { "red", "blue", "yellow", "gold", "silver", "crystal", "ruby" }
...
-- 1, 2, or 3.  The mod API is shared across generations (same hook names, same
-- registry names), so the pieces that must branch -- the manifest gen2compat
-- gate, the registry target routing, the mod.world arm -- ask this rather than
-- each spelling out its own isGold() test.  A new generation adds a
-- `generation` to its VERSIONS row and nothing else changes shape.
function GameVersion.generation(id)
  return GameVersion.info(id).generation or 1
end

-- "gen1" | "gs" | "crystal" | "gen3": lineage within a generation.
