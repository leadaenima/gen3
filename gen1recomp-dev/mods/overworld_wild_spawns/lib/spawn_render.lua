...
-- Gen 3's sprites schema wants path/width/height/frameCount + numeric id
-- (Schemas.R.sprites.gen3Fields). Gen 1/2 accept image/frames. Translate
-- here so the same registerContent path works on Ruby.
function SpawnRender:_normalizeSpriteDef(id, def)
  if type(def) ~= "table" then return def end
  local gen = 1
  do
    local ok, GV = pcall(require, "src.core.GameVersion")
    if ok and GV and type(GV.generation) == "function" then
      gen = tonumber(GV.generation(GV.current)) or tonumber(GV.generation()) or 1
    end
  end
  if gen ~= 3 then return def end
  if type(def.path) == "string" and type(def.width) == "number"
      and type(def.height) == "number" and type(def.frameCount) == "number"
      and type(def.id) == "number" then
    return def
  end
  local path = def.path or def.image
  if type(path) ~= "string" or path == "" then return def end
  local w = tonumber(def.width) or 16
  local h = tonumber(def.height) or 16
  local frames = tonumber(def.frameCount) or tonumber(def.frames) or 1
  if frames < 1 then frames = 1 end
  local numericId = tonumber(def.id)
  if not numericId then
    local acc = 0
    local key = tostring(id or path)
    for i = 1, #key do
      acc = (acc * 33 + key:byte(i)) % 10000
    end
    numericId = 50000 + acc
  end
  return {
    ...
    height = h,
    frameCount = frames,
    face = def.face or { down = 0, up = 0, left = 0, right = 0 },
    walk = def.walk or {
...
function SpawnRender:_registerSprite(id, def)
  ...
  local normalized = self:_normalizeSpriteDef(id, def)
  local ok, err = pcall(function()
    self.mod.content.sprites:register(id, normalized)
  end)
  if not ok then
