...
base=json.loads((R/'tools/baselines/8.1.30-runtime-sha256.json').read_text())['files']
allowed={
    'lib/voxel_atmos/ConnectedWater3D.lua',
    'lib/DramalessAtmos.lua',
    'lib/ConnectedWater.lua',
    'lib/Settings.lua',
...
for rel,exp in base.items():
    ...
    raw=p.read_bytes();got=(hashlib.sha256(raw).hexdigest(),len(raw));want=(exp['sha256'],exp['size'])
...
print(f'8.1.31 runtime preservation: changed={sorted(changed)} unexpected={sorted(set(bad))}')
