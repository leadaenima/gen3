...
base=json.loads((R/'tools/baselines/8.1.48-runtime-sha256.json').read_text())['files']
allowed={'lib/Settings.lua','lib/SettingsMenu.lua'}
changed=[]; bad=[]
...
for rel in sorted(allowed-set(changed)):
    print('FAIL intended settings delta missing '+rel); bad.append(rel)
print(f'8.1.49 runtime preservation: changed={sorted(changed)} unexpected={sorted(set(bad))}')
