...
allowed={'lib/ProceduralSnowField.lua'}
changed=[];bad=[]
for rel,exp in base.items():
    p=R/rel
    if not p.exists(): print('FAIL missing 8.1.54 runtime '+rel);bad.append(rel);continue
...
for rel in sorted(allowed-set(changed)): print('FAIL required snow-motion runtime change missing '+rel);bad.append(rel)
print(f'8.1.55 runtime delta: changed={sorted(changed)} unexpected={sorted(set(bad))}')
