...
base=json.loads((R/'tools/baselines/8.1.60-runtime-sha256.json').read_text())['files']
allowed={'lib/Tornado.lua'}
changed=[];bad=[]
...
for rel in sorted(allowed-set(changed)): print('FAIL required 8.1.61 change missing '+rel);bad.append(rel)
print(f'8.1.61 runtime delta changed={sorted(changed)}')
