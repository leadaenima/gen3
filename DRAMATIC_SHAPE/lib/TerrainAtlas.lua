...
local function staticAtlas(map, colors)
  ...
  -- it on a THIRD path: its tilesets carry a palette per tile graphic
  -- (palMap/palColors, off the ROM), and TileRenderer.new bakes those into
  -- renderer.image the same way RED++ bakes its own -- so the atlas in hand
  -- is already the map's colour. Reading only the tileset flag missed that
  -- and sent Johto down the SGB branch below, which repaints the RAW
...
local function newEntry(map, base, baked)
  ...
  local ok, entry = pcall(function()
    ...
    local renderer = map.renderer
    local recolored = baked or (renderer and (renderer.gbcAtlas
                                              or renderer.trueColor))
